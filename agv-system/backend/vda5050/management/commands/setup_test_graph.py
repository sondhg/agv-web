    ...
    def handle(self, *args, **options):
        ...
        nodes_data = [
            # 1. BÃI SẠC PIN (Depot / Charging Zone)
            {
                ...
                "x": 0,
                "y": 0,
                "type": GraphNode.NodeType.CHARGING,
            },
            {
                "node_id": "Charge_02",
                "x": 0,
            ...
            },
            {
                ...
                "x": 8,
                "y": 4,
                "type": GraphNode.NodeType.DEFAULT,
            },  # Cổng bãi sạc
            {
                "node_id": "Depot_Buffer",
                "x": 16,
                "y": 10,
                "type": GraphNode.NodeType.DEFAULT,
            },
            ...
            # 4. KHU VỰC LẤY HÀNG (Warehouse - Pickup)
            {
                ...
                "x": 45,
                "y": 25,
                "type": GraphNode.NodeType.PICKUP,
            },
            {
                "node_id": "WH_Pick_2",
                "x": 45,
            ...
            # 5. KHU VỰC DÂY CHUYỀN SẢN XUẤT (Assembly Line - Delivery)
            {
                ...
                "x": -15,
                "y": 55,
                "type": GraphNode.NodeType.DELIVERY,
            },
            {
                "node_id": "Assy_Drop_2",
                "x": -15,
        ...
        for node_data in nodes_data:
            ...
            self.stdout.write(
                f"  ✅ Created: {styled_type.ljust(20)} {node_data['node_id'].ljust(15)} at ({node_data['x']:>3}, {node_data['y']:>3})"
            )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"  ✅ Created {edge_count} directed edges successfully."
            )
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"\n{'=' * 70}\n✅ Large Map Setup Complete!\n{'=' * 70}\n"
            )
        )

        self.stdout.write("📊 Factory Map Topology (optimized multi-lane):\n")
        self.stdout.write(
            f"  {self.style.SUCCESS('[Assy_Drop_2]')} - [West_N] = [Main_N] = [Aisle_N] = [Aisle2_N] - {self.style.SUCCESS('[WH_Pick_3]')}"
        )
        self.stdout.write("                     ||       ||         ||         ||")
        self.stdout.write(
            f"  {self.style.SUCCESS('[Assy_Drop_1]')} - [West_C] = [Main_C] = [Aisle_C] = [Aisle2_C] - {self.style.SUCCESS('[WH_Pick_2]')}"
        )
        self.stdout.write("                     ||       ||         ||         ||")
        self.stdout.write(
            f"                  [West_S] = [Main_S] = [Aisle_S] = [Aisle2_S] - {self.style.SUCCESS('[WH_Pick_1]')}"
        )
        self.stdout.write("                     ||         ||         ||")
        self.stdout.write("                [Depot_Gate] = [Depot_Buffer]")
        self.stdout.write("                 /          \\")
        self.stdout.write(
            f"      {self.style.WARNING('[Charge_01]')}          {self.style.WARNING('[Charge_02]')}\n"
        )

        self.stdout.write(
            f"📈 Map Stats: {GraphNode.objects.count()} Nodes, {GraphEdge.objects.count()} Edges."
        )
        self.stdout.write(
            "   * [===] = High Speed (2.0 m/s)  |  [---] = Normal/Low Speed (1.0 - 1.5 m/s)"
        )
        self.stdout.write(
            self.style.SUCCESS(
