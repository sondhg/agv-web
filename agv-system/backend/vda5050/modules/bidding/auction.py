...
class AuctionCoordinator:
    ...
    def run_auction(
        ...
        bids = self.collect_bids(
            agvs, pickup_node_id, delivery_node_id, load_kg, epsilon=epsilon
        )
        ...
        if AUCTION_ALGORITHM == "SSI_MARGINAL":
            ...
            logger.info(
                f"   Distance to Pickup: {winner_details['distance_to_pickup_m']:.2f}m"
            )
        elif AUCTION_ALGORITHM == "GREEDY_ETA":
            logger.info(
                f"   ETA: {winner_details.get('eta_s', winner_details['bid_final']):.2f}s"
            )
            logger.info(
                f"   Queue Time: {winner_details.get('queue_time_s', 0.0):.2f}s"
            )
