...
class SsiMarginalBidStrategy:
    ...
    def _calculate_single_leg_marginal(
        self, agv, start_node, pickup_node_id, load_kg, wait_time
    ):
        """Calculate marginal metrics for single-leg trip: start -> pickup."""
        actual_distance, actual_turns = self.calculator.graph_engine.get_path_info(
        ...
        energy_travel, time_travel = (
            self.calculator.transport_calculator.calculate_metrics(
    ...
    def calculate_bid_score(self, marginal_cost_result, epsilon=None):
        ...
        bid_minimax = K_ENERGY * (norm_energy + norm_queue_energy) + K_TIME * (
            norm_time + norm_queue_time
        )
