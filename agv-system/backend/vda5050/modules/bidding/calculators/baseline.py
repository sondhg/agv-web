...
from ...constant import DEFAULT_LOAD_KG, FALLBACK_NORM_ENERGY_KJ, FALLBACK_NORM_TFT_SEC
...
class BaselineCalculator:
    ...
    def calculate_baseline_distance(self, start_node_id, target_node_id):
        ...
        """
        distance, num_turns = self.graph_engine.get_path_info(
        ...
        if distance == float("inf"):
            ...
            logger.debug(
                f"Baseline {start_node_id}→{target_node_id}: {distance:.2f}m, {num_turns} turns"
            )

        return distance, num_turns

    def calculate_baseline_metrics(
        self, start_node_id, target_node_id, load_kg=DEFAULT_LOAD_KG
    ):
        """
        ...
        """
        distance, num_turns = self.calculate_baseline_distance(
            start_node_id, target_node_id
        )

        if distance == float("inf"):
            logger.warning(
                f"Using fallback baseline values for {start_node_id}→{target_node_id}"
            )
            return FALLBACK_NORM_ENERGY_KJ, FALLBACK_NORM_TFT_SEC

        energy_kj, time_s = self.transport_calculator.calculate_metrics(
            distance, load_kg, num_turns
        )

        validated_energy, validated_time = self.transport_calculator.validate_metrics(
            energy_kj, time_s
        )

        logger.debug(
            f"Baseline metrics {start_node_id}→{target_node_id}: "
    ...
    def normalize_metrics(
        self, actual_energy_kj, actual_time_s, baseline_energy_kj, baseline_time_s
    ):
        """
        ...
        """
        # Tránh chia cho 0
        safe_baseline_energy = (
            baseline_energy_kj if baseline_energy_kj > 0 else FALLBACK_NORM_ENERGY_KJ
        )
        safe_baseline_time = (
            baseline_time_s if baseline_time_s > 0 else FALLBACK_NORM_TFT_SEC
        )
        ...
        logger.debug(
            f"Normalized metrics: E_norm={norm_energy:.4f}, T_norm={norm_time:.4f}"
        )

        return norm_energy, norm_time

    def calculate_and_normalize(
        self, start_node_id, target_node_id, actual_distance_m, load_kg=DEFAULT_LOAD_KG
    ):
        """
        ...
        norm_energy, norm_time = self.normalize_metrics(
            actual_energy, actual_time, baseline_energy, baseline_time
        )
